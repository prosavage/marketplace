...
export default function RecentSalesChart() {
    ...
    const aboutAWeekAgo = new Date(new Date().getTime() - 60 * 60 * 24 * 7 * 1000)

    const [start, setStart] = useState(
        aboutAWeekAgo.toLocaleDateString()
    );

    const [end, setEnd] = useState(new Date().toLocaleDateString());

    // for when we have no data we can show empty chart.
    const generateDummyData = () => {
        const dummyData: ChartDataNode[] = [];
        const now = new Date()
        let starter = new Date(aboutAWeekAgo);
        const ONE_DAY = 60 * 60 * 24 * 1000
        while (starter.getTime() <= now.getTime()) {
            dummyData.push({sales: 0, amount: 0, date: starter.toLocaleDateString()})
            starter = new Date(starter.getTime() + ONE_DAY)
        }
    ...
    useEffect(() => {
        ...
        getAxios()
            ...
            .then((res) => {
                const payments = res.data.payload.payments;
