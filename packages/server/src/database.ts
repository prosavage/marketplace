...
const ensureIndexes = async () => {
    ...
    await getDatabase().collection(RESOURCES_COLLECTION).createIndex({ type: 1, category: 1 })


    // resource indexer for reviews.
    await getDatabase().collection(REVIEWS_COLLECTION).createIndex({ resource: 1 });
    // resource indexer for versions.
    await getDatabase().collection(VERSIONS_COLLECTION).createIndex({ resource: 1 })
}


export const pageSearchCollectionWithFilter = async (collection: string, filter: object, page: number) => {
    const limit = 10
    return await getDatabase().collection(collection).find(filter)
        .sort({ updated: -1 })
        .skip((page - 1) * limit)
        .limit(limit)
        .toArray();
...
export const updateResourceRating = async (resourceId: Resource["_id"]) => {
    const reviews = await getDatabase().collection<Review>(REVIEWS_COLLECTION).find({ resource: resourceId }).toArray();
    if (!reviews) {
        console.log(`Resource ${resourceId}'s update was cancelled due to no reviews being found.`)
        return;
    }
    ...
    for (const author of authors) {
        ...
        const latestReview = authorReviews.sort((a, b) => a.timestamp < b.timestamp ? 1 : -1)[0]
        if (!latestReview) {
    ...
    const sum = calculatedReviews.map(review => review.rating).reduce((total, current) => total + current);
    const avg = Math.round(sum / calculatedReviews.length);
    await getDatabase().collection(RESOURCES_COLLECTION).updateOne({ _id: resourceId }, { rating: avg })
}
