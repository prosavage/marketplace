import { Payment, PaymentStatus, User } from "@savagelabs/types";
  ...
  async (req, res) => {
    ...
    if (event.type === "payment_intent.succeeded") {
      ...
      const payment = await getDatabase()
        ...
        .findOne({ payment_intent });
      if (payment === null) {
        console.log("Invalid payment_intent...");
        console.log("Something was went wrong.");
        console.log({ paymentIntent });
        res.failure(
          `payment not found in database. payment_intent: ${payment_intent}`
        );
        return;
      }
      if (!charge.paid) {
      ...
      if (!recipient.purchases.includes(payment.resource)) {
        await getDatabase()
          ...
          .updateOne(
            { _id: payment.user },
            { $push: { purchases: payment.resource } }
          );
      }
      ...
      await getDatabase()
        ...
        .updateOne(
          { _id: payment._id },
          { $set: { status: PaymentStatus.CONFIRMED } }
        );
